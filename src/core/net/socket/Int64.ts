/**
 * 用法:
 *1.parseData或者fromBytes读取二进制里面的int64数据；或读取字符串的值fromString
 *2.toData,将数值写入egret.ByteArray
 *3.copy, clone, cloneTo,克隆
 *4.equals比较两个long类型在数值上是否相等。
 */
class Int64 {
    ...
    /** 10进制右移操作*/
    public rightShiftDecimal(divisor:number): string {
        let valueStr:string = this.toString();
        let times:number = 0;
        while (divisor >= 10)
        {
            divisor /= 10;
