...
class ResourceUtils extends BaseClass {
    ...
    private onResourceLoadError(event: RES.ResourceEvent): void {
        Log.trace(Log.LOAD, event.groupName + "资源组有资源加载失败");
     ...
     * @param $onResourceLoadTarget 资源加载监听函数所属对象
     */
    public loadResource($resources = [], $groups = [], $onResourceLoadComplete: Function = null, $onResourceLoadProgress: Function = null, $onResourceLoadTarget: any = null, $groupName: string = null): void {
        var needLoadArr = $resources.concat($groups);
        var groupName;
        if ($groupName == null) {
            groupName = "loadGroup" + this._groupIndex++;
        } else {
            groupName = $groupName;
        }
     ...
     * @param {string} resUrl
     */
    public createResource(resKey: string, resType: string, resUrl: string):any {
        var res: any = {
            name: resKey,
            type: resType,
            url: resUrl
