class MgShapeChangeExConfig extends BaseConfig{
	/**最大等级字典 */
	...
	public constructor() {
		super("t_mg_shape_change_ex", "shape,change,level");
	}
	...
	public static getAttrListData(eshape: EShape, change: number, shapeLevel: number, isInCludeNext: boolean = true): Array<AttrInfo> {
		...
		let maxLevel: number = ConfigManager.mgShapeChangeEx.getMaxLevel(eshape, change);
		...
		cfg = ConfigManager.mgShapeChangeEx.getByShapeChangeAndLevel(eshape, change, level);
		...
		if (isInCludeNext) {
			...
			nextCfg = ConfigManager.mgShapeChangeEx.getByShapeChangeAndLevel(eshape, change, nextLevel);
		...
		if(!this.skillDict || !this.talentSkillDict){
			...
			for(let key in dict){
				...
				if(data.openSkill){
					...
					this.skillDict[data.shape][data.change][data.openSkill] = data.level ? data.level : 0;
				}
				if(data.openTalentSkillIndex){
					...
					this.talentSkillDict[data.shape][data.change][data.openTalentSkillIndex] = data.level ? data.level : 0;
				}
			}
		}
	}
