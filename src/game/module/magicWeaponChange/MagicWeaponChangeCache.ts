class MagicWeaponChangeCache implements ICache{
	private shape: number;

	public constructor() {
		this.shape = EShape.EShapeSpirit;
	}
	...
	public getChangesData(shape: EShape): Array<any>{
		let changeDict: any = ConfigManager.mgShapeChange.getChangeDict(shape);
		...
		for(let changeCode in changeDict){
			...
			if(changeEx){
				changeCfg = ConfigManager.mgShapeChange.getByShapeChangeAndLevel(shape, Number(changeCode), changeEx.level_I);
				...
				changeCfg = ConfigManager.mgShapeChange.getByShapeChangeAndLevel(shape, Number(changeCode), 0);
				changeData["iaActived"] = false;
				if(changeCfg.defaultShow){
					changeData["level"] = -1;
					changeData["cfg"] = changeCfg;
					changeArr.push(changeData);
				}else if(CacheManager.pack.propCache.getItemCountByCode(changeCfg.activeCode) > 0){
					changeData["level"] = -1;
					changeData["cfg"] = changeCfg;
					changeArr.push(changeData);
				}
	...
	public getTalentSkill(change: number): any{
		...
		if(!skillIndex){
			skillIndex = ConfigManager.mgShapeChange.getChangeTalentSkill(this.shape, change);
		}
		...
		data["isChange"] = true;
		if(!data.isOpen){
			openLevel = ConfigManager.mgShapeChange.getOpenLevel(this.shape, change, skillIndex, data.isTalent);
			data["openLevelStr"] = ConfigManager.mgShapeChange.getStageStar(this.shape, change, openLevel);
		}else{
	...
	public getSkills(change: number): Array<any>{
		let skillIds: Array<number> = ConfigManager.mgShapeChange.getChangeSkillArr(this.shape, change);
		...
		for(let skill of skillIds){
			...
			data["isChange"] = true;
			if(!data.isOpen){
				openLevel = ConfigManager.mgShapeChange.getOpenLevel(this.shape, change, skill, data.isTalent);
				data["openLevelStr"] = ConfigManager.mgShapeChange.getStageStar(this.shape, change, openLevel);
			}
			skillData.push(data);
		}
	...
	public isCanActive(change: number): boolean{
		// let changeCfg: any = ConfigManager.mgShapeChange.getByShapeChangeAndLevel(this.shape, change, 1);
	...
	public checkTipsByChangeData(change: number, data: any): boolean{
		if(data.isActived){
			let count: number = CacheManager.pack.propCache.getItemCountByCode(data.cfg.useItemCode);
			let costNum: number = data.cfg.useItemNum ? data.cfg.useItemNum : 0;
			if((count >= costNum && data.level < ConfigManager.mgShapeChange.getMaxLevel(this.shape, data.change))){
				return true;
			}else if(this.isCanUpgradeTSkill(data.change)){
				return true;
			}
		}else{
			if(CacheManager.pack.propCache.getItemCountByCode(data.cfg.activeCode) >= data.cfg.activeNum){
				return true;
			}
		}
		return false;
