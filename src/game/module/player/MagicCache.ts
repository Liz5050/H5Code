class MagicCache extends BaseShapeCache{
	...
	public setMaterials():void{
		...
		materials = ConfigManager.item.selectCTShapeAndColor(ECategory.ECategoryMaterial, EProp.EPropEquipRefineMaterial, EShape.EShapeMagic, EColor.EColorBlue, EColor.EColorOrange);
	...
	public setDrugs():void{
		this.drugItems = [];
		let drugs:any;
		drugs = ConfigManager.mgShapeDrugAttr.getByPk(EShape.EShapeMagic);
		for(let i = 0; i < 3; i++){
	...
	public setSkills():void{
		this.skillItems = [];
		let skills:any;
		skills = ConfigManager.mgShapeOpen.select({"shape": EShape.EShapeMagic});
	...
	public setPropertys():void{
		...
		let shapeItem:any;
		let property:any;
		this.shapeItem = ConfigManager.mgShape.getByShapeAndLevel(EShape.EShapeMagic, this.level);
		property = WeaponUtil.getAttrDict(this.shapeItem.attrList);
		let nextProperty: any = null;
		if(this.level < 400){
			nextProperty = WeaponUtil.getAttrDict((ConfigManager.mgShape.getByShapeAndLevel(EShape.EShapeMagic, this.level + 1)).attrList);
		}
		...
		for(let key in property){
			...
			if(GameDef.EJewelName[key][0] == GameDef.EJewelName[EJewel.EJewelCritRate][0]){
				let toughValue : number = 0;
				for(let attrValue of this.drugsAttr){
					toughValue += (attrValue["attrList"][EJewel.EJewelToughness] ? attrValue["attrList"][EJewel.EJewelToughness] : 0) * attrValue["amount"];
				}
				if(this.drugsAttr[1]["amount"] > 0){
					toughValue *= (1+this.drug2AttrRate/10000*this.drugsAttr[1]["amount"]);
				}
				this.propertyItems.push({"name": GameDef.EJewelName[EJewel.EJewelToughness][0], "value": Math.ceil(toughValue), "typeIncrease": 0});
			}
