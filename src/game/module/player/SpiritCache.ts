class SpiritCache extends BaseShapeCache{
	...
	public setMaterials():void{
		...
		materials = ConfigManager.item.selectCTShapeAndColor(ECategory.ECategoryMaterial, EProp.EPropEquipRefineMaterial, EShape.EShapeSpirit, EColor.EColorBlue, EColor.EColorOrange);
	...
	public setDrugs():void{
		this.drugItems = this.getDrugs(EShape.EShapeSpirit);		
	}
	...
	public setSkills():void{
		this.skillItems = [];
		let skills:any;
		skills = ConfigManager.mgShapeOpen.select({"shape": EShape.EShapeSpirit});
	...
	private setDrugsValue(){
		this.drugsAttr = [];
		let drugs:any;
		drugs = ConfigManager.mgShapeDrugAttr.getByPk(EShape.EShapeSpirit);
	...
	public setPropertys():void{
		...
		this.drugItemMax = [];
		let property:any;
		this.shapeItem = ConfigManager.mgShape.getByShapeAndLevel(EShape.EShapeSpirit, this.level);
		property = WeaponUtil.getAttrDict(this.shapeItem.attrList);
		let nextProperty: any = null;
		if(this.level < 400){
			nextProperty = WeaponUtil.getAttrDict((ConfigManager.mgShape.getByShapeAndLevel(EShape.EShapeSpirit, this.level + 1)).attrList);
		}
