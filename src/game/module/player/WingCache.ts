class WingCache extends BaseShapeCache{
	...
	public setMaterials():void{
		...
		materials = ConfigManager.item.selectCTShapeAndColor(ECategory.ECategoryMaterial, EProp.EPropEquipRefineMaterial, EShape.EShapeWing, EColor.EColorBlue, EColor.EColorOrange);
	...
	public setDrugs():void{
		this.drugItems = this.getDrugs(EShape.EShapeWing);		
	}
	...
	public setSkills():void{
		this.skillItems = [];
		let skills:any;
		skills = ConfigManager.mgShapeOpen.select({"shape": EShape.EShapeWing});
	...
	private setDrugsValue(){
		this.drugsAttr = [];
		let drugs:any;
		drugs = ConfigManager.mgShapeDrugAttr.getByPk(EShape.EShapeWing);

		// this.drugsAttr.push({"attrList":WeaponUtil.getAttrDict(drugs.drug1PerAttrList), "amount":0});
		// this.drugsAttr.push({"attrList":WeaponUtil.getAttrDict(drugs.drug2PerAttrList), "amount":0});
		// this.drugsAttr.push({"attrList":WeaponUtil.getAttrDict(drugs.drug3PerAttrList), "amount":0});
	...
	public setPropertys():void{
		...
		// let shapeItem:any;
		let property:any;
		this.shapeItem = ConfigManager.mgShape.getByShapeAndLevel(EShape.EShapeWing, this.level);
		property = WeaponUtil.getAttrDict(this.shapeItem.attrList);
		let nextProperty: any = null;
		if(this.level < 400){
			nextProperty = WeaponUtil.getAttrDict((ConfigManager.mgShape.getByShapeAndLevel(EShape.EShapeWing, this.level + 1)).attrList);
		}
