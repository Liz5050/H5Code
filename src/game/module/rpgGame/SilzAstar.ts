class SilzAstar {
     ...
     */
    public find(xnow: number, ynow: number, xpos: number, ypos: number, moveType: EMoveType = EMoveType.EMoveTypeNormal): PathNode[] {
        let startGridX = Math.floor(xnow / RpgGameData.GameCellWidth);
        let startGridY = Math.floor(ynow / RpgGameData.GameCellHeight);
        let endGridX = Math.floor(xpos / RpgGameData.GameCellWidth);
        let endGridY = Math.floor(ypos / RpgGameData.GameCellHeight);

        if (moveType == EMoveType.EMoveTypeRush) { //冲刺的话，直线寻路
            let rushPoints: Array<egret.Point> = App.MathUtils.getLineSegmentPoints(startGridX, startGridY, endGridX, endGridY);
            this.getLineSegmentPath(rushPoints);
            return this._path;
        } else if (moveType == EMoveType.EMoveTypeJump) { //跳跃的寻路处理
            this.processJumpPath(startGridX, startGridY, endGridX, endGridY);
            return this._path;
        }
        else {
            this._path = PathUtils.findBeelinePath(xnow, ynow, xpos, ypos);
            if (this._path && this._path.length) {
                return this._path;
            }

            this._grid.setStartNode(startGridX, startGridY); //1
            this._grid.setEndNode(endGridX, endGridY); //2
    ...
    public findJumpPath(xpos: number, ypos: number): PathNode[] {
        let _path:PathNode[] = [];
        if (this.gridXYCanWalk(xpos, ypos)) {
            _path.push(new PathNode(xpos, ypos, true));
        }
        return _path;
    }
     ...
     * 测试：先直接根据目标点来算，即只有2个
     */
    private processJumpPath(x1: number, y1: number, x2: number, y2: number): void {
        this._path = [];
        if (this.gridXYCanWalk(x1, y1)) {
            this._path.push(new PathNode(x1, y1));
        }
        if (this.gridXYCanWalk(x2, y2)) {
            this._path.push(new PathNode(x2, y2, true));
        }

        // //打印测试
        // for (let path of this._path) {
        //     Log.trace("processJumpPath打印：", path.toString());
        // }
    }

    private getLineSegmentPath(listPoint: Array<egret.Point>): void {
        this._path = [];
        let point: egret.Point;
        for (let i: number = 0; i < listPoint.length; i++) {
            point = listPoint[i];
            if (this.gridXYCanWalk(point.x, point.y)) {
                this._path.push(new PathNode(point.x, point.y));
            }
        }
    }

    public canLineRush(xnow: number, ynow: number, xpos: number, ypos: number): boolean {
        ...
        let rushPoints: Array<egret.Point> = App.MathUtils.getLineSegmentPoints(xnow, ynow, xpos, ypos);
        let point: egret.Point;
        for (let i: number = 0; i < rushPoints.length; i++) {
            point = rushPoints[i];
            if (!this.gridXYCanWalk(point.x, point.y)) {
                // Tip.showRollTip("有不可寻路点，不能冲刺");
                return false;
            }
        }
        return true;
    ...
    public gridXYCanWalk(x: number, y: number): boolean {
        if (x >= this._grid.numCols || y >= this._grid.numRows || x < 0 || y < 0) {
            return false;
        }
        x = Math.min(x, this._grid.numCols - 1);
        y = Math.min(y, this._grid.numRows - 1);

        if (!this._grid.getNode(x, y)) {
            return false;
        }

        if (!this._grid.getNode(x, y).walkable) {
            return false;
        }
        return true;
    }
    ...
    public pixelXYCanWalk(x: number, y: number): boolean {
        x = Math.floor(x / RpgGameData.GameCellWidth);
        y = Math.floor(y / RpgGameData.GameCellHeight);
        return this.gridXYCanWalk(x, y);
    }

    public get absoluteRadian(): number {
        if (this._grid && this._grid.startNode && this._grid.endNode) {
            return App.MathUtils.getRadian2(this._grid.startNode.x, this._grid.startNode.y, this._grid.endNode.x, this._grid.endNode.y);
            // return App.MathUtils.getDistance(this._grid.startNode.x, this._grid.startNode.y, this._grid.endNode.x, this._grid.endNode.y);
        }
        return null;
    }
    ...
    public makeGrid(data: number[][]): void {
        ...
        for (py = 0; py < rows; py++) {
            for (px = 0; px < cols; px++) {
                this._grid.setWalkable(px, py, data[py][px] >= 5);
            }
        }
...
class PathNode {
    ...
    /** 特殊数字，比如跳跃等要用 */
    public specialNo: number = 0;
    public role_dir: number = -1;
    public pX:number;
    public pY:number;

    //public index:number;
    public constructor(x: number, y: number, isJump: boolean = false, specialNo: number = 0, role_dir: number = -1) {
        this.x = x;
        this.y = y;
        this.isJump = isJump;
    ...
    public setSPos(px:number, py:number):void {
        this.pX = px;
        this.pY = py;
    }
